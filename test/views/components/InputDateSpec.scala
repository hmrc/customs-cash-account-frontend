...
class InputDateSpec extends SpecBase {
  "InpuDate component" should {
    ...
    "render correctly with no errors" in new Setup {
      ...
      running(app) {
        val inputDateView =
          app.injector.instanceOf[inputDate]
    ...
    "render correctly with month error" in new Setup {
      ...
      running(app) {
        ...
        html.getElementById(s"$id.year").attr(id) must include("2021")
        html.getElementsByTag("input").get(1).attr("class") must include(
          "govuk-input--error"
        )
      }
    }
    ...
    "render correctly with year error" in new Setup {
      ...
      running(app) {
        ...
        html.getElementsByTag("input").get(2).attr("class") must include(
          "govuk-input--error"
        )
      }
    }
    ...
    "render correctly with both day, month and year errors" in new Setup {
      ...
      running(app) {
        ...
        html
          ...
          .attr("class") must include("govuk-input--error")
      }
    }
