...
import connectors.{
  CustomsFinancialsApiConnector, ErrorResponse, NoTransactionsAvailable, SdesConnector, TooManyTransactionsRequested
}
...
class CashAccountV2Controller @Inject() (
  ...
  def onSubmit(page: Option[Int]): Action[AnyContent] = (authenticate andThen verifyEmail).async { implicit request =>
    form
      .bindFromRequest()
      .fold(
        formWithErrors => processAccountDetails(formWithErrors, page),
        enteredValue =>
          Future.successful {
  ...
  private def processAccountDetails(form: Form[String] = formProvider(), page: Option[Int])(implicit
  ...
  private def processErrorResponse(
    account: CashAccount,
  ...
  private def getStatementsForEori(eori: String, from: LocalDate, to: LocalDate)(implicit
    hc: HeaderCarrier,
    request: RequestHeader,
    ec: ExecutionContext
  ): Future[Seq[CashStatementsForEori]] =
    sdesConnector
      .getCashStatements(eori)
      .map { statementFiles =>
        val eoriHistory: EoriHistory = EoriHistory(eori, Some(from), Some(to))

        val groupedStatements = statementFiles
