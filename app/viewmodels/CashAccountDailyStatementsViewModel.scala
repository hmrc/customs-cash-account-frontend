...
import models.{
  CashDailyStatement, CashTransactionType, CashTransactions, Declaration, Payment, Transaction, Transfer, Withdrawal
}
...
case class PaymentType(mrnLink: Option[HtmlFormat.Appendable] = None, textString: Option[String] = None)

case class DailyStatementViewModel(
  date: LocalDate,
  transactionType: Option[PaymentType] = None,
  credit: Option[String] = None,
  debit: Option[String] = None,
  balance: Option[String]
) extends Ordered[DailyStatementViewModel] {
  override def compare(that: DailyStatementViewModel): Int = that.date.compareTo(date)
}

case class CashAccountDailyStatementsViewModel(
...
object CashAccountDailyStatementsViewModel {
  def apply(transactions: CashTransactions, pageNo: Option[Int])(implicit
    msgs: Messages,
    config: AppConfig
  ...
  private def populateDailyStatementViewModelList(
    dailyStatements: Seq[CashDailyStatement]
  )(implicit msgs: Messages): Seq[DailyStatementViewModel] = {

    val result: Seq[Seq[DailyStatementViewModel]] = dailyStatements.map { dStat =>
      val date = dStat.date
      ...
      val closingBalanceOnlyRow =
        DailyStatementViewModel(date = dStat.date, balance = Some(formatCurrencyAmount(dStat.closingBalance)))
  ...
  private def populateViewModelFromDeclarations(date: LocalDate, declarations: Seq[Declaration])(implicit
    msgs: Messages
  ): Seq[DailyStatementViewModel] =
            ...
            Some(
              linkComponent(
                LinkComponentValues(
                  ...
                  pWrapped = false,
                  location = controllers.routes.DeclarationDetailController
                    .displayDetails(declaration.secureMovementReferenceNumber.getOrElse(emptyString), None)
                    .url
                )
              )
            )
  ...
  private def populateViewModelFromPaymentAndWithdrawals(date: LocalDate, paymentAndWithdrawals: Seq[Transaction])(
    implicit msgs: Messages
  ): Seq[DailyStatementViewModel] =
    paymentAndWithdrawals.map { paymentAndWithdrawal =>
      DailyStatementViewModel(
        date = date,
