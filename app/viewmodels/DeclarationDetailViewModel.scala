...
import utils.Utils.{emptyH1InnerComponent, emptySpanHeadingComponent, emptyString}
...
object DeclarationDetailViewModel {
  ...
  def apply(account: CashAccount, eori: String, declaration: Declaration)(implicit
    messages: Messages
      ...
      rows = Seq(
            ...
            HtmlContent(
              Formatters.formatCurrencyAmount(
                declaration.taxGroups
                  .find(_.taxGroupDescription == CustomsDuty)
                  .map(_.amount)
        ...
        SummaryListRow(
          ...
          value = Value(content =
            HtmlContent(
              Formatters.formatCurrencyAmount(
                declaration.taxGroups
                  .find(_.taxGroupDescription == ImportVat)
                  .map(_.amount)
                  .getOrElse(BigDecimal(0))
              )
            )
        ...
        SummaryListRow(
          ...
          value = Value(content =
            HtmlContent(
              declaration.taxGroups
                .find(_.taxGroupDescription == ExciseDuty)
                .map(_.amount)
